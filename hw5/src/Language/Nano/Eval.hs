...
-- *** Exception: Error {errMsg = "type error: binop"}
--
-- >>> let e1 = let e1 = EIf (EBin Lt (EVar"z1") (EVar "x") ) (EBin Ne (EVar "y") (EVar "z")) (EBool False)
-- >>> eval env0 e1
-- True
--
...
eval env (EIf e1 e2 e3)     = if (eval env e1) == (VBool True) then (eval env e2) else (eval env e3)
...
eval env (EApp f a)         = case eval env f of
                              ...
                              x                    -> throw ( Error ("Type Error:" ++ (show x)))


head_ :: Value -> Value
head_ (VPair a _) = a
head_ _ = throw (Error "type error")



tail_ :: Value -> Value
tail_ (VPair _ b) = b
tail_ _ = throw (Error "type error")
...
evalOp Ne (VPair a b) (VPair x y)= (evalOp Or (evalOp Ne a x) (evalOp Ne b y))
evalOp Ne _ _                    = VBool True
...
evalOp Cons a VNil               = VPair a VNil
evalOp Cons a (VPair x y)        = if (type_ a) == (type_ x) then VPair a b' else throw (Error "type error")
                                 where b' = VPair x y
...
--------------------------------------------------------------------------------
lookupId id []     = throw (Error ("unbound variable: " ++ id) ) -- error "TBD:lookupId"
lookupId id (x:xs) = if fst x == id then snd x else lookupId id xs
