...
eval env expr = case expr of
  (ENil)         -> VNil
  (EInt i)       -> VInt i                               -- Create value from int
  (EVar id)      -> lookupId id env                      -- lookup id
  (EBool b)      -> VBool b                              -- Create values from bool
  (EBin b e1 e2) -> evalOp b (eval env e1) (eval env e2) -- evalulate bool AFTER eval sub e1 & e2
  (EIf b e1 e2)  -> case (eval env b) of 
                      (VBool bool) -> if(bool) then (eval env e1) else (eval env e2)
                      (VErr m) -> VErr m
                      (_) -> VErr ("type error: elif") -- Not bool type, throw error
  (ELet id e1 e2)   -> eval env' e2                         -- evalulate with complete env
                        where env' = (id, eval env' e1):env -- Add id to env and solve recursively
  (ELam x body)     -> VClos env x body                     -- Create enclosure (to prevent dynamic vars)
  (EApp fun arg)    -> case (eval env fun) of
                        (VPrim p) -> p (eval env arg)
                        (VClos closEnv x body) -> eval bodyEnv body
                                                    where
                                                      (VClos closEnv x body) = eval env fun -- If more "levels", eval that vclos
                                                      bodyEnv = (x,(eval env arg)):closEnv  -- Create new enviroment
                        (VErr m) -> VErr m
                        (_)                    -> VErr ("type error: EApp")         -- invalid type

--------------------------------------------------------------------------------
evalOp :: Binop -> Value -> Value -> Value
--------------------------------------------------------------------------------
evalOp Cons v1 v2 = VPair v1 v2 -- Create list (aka pairs with pairs)

evalOp b (VInt v1) (VInt v2) = case b of
  Plus  -> VInt (v1 + v2)   --  + for int types
  Minus -> VInt (v1 - v2)   --  - for int types
  Mul   -> VInt (v1 * v2)   --  * for int types
  Eq    -> VBool (v1 == v2) -- == for int types
  Ne    -> VBool (v1 /= v2) -- /= for int types
  Lt    -> VBool (v1 < v2)  --  < for int types
  Le    -> VBool (v1 <= v2) -- <= for int types

evalOp b (VBool b1) (VBool b2) = case b of
  Eq    -> VBool (b1 == b2) -- == for bool types
  Ne    -> VBool (b1 /= b2) -- /= for bool types
  And   -> VBool (b1 && b2) -- && for bool types
  Or    -> VBool (b1 || b2) -- || for bool types

evalOp Eq v1 v2 = VBool (v1 == v2)
evalOp Ne v1 v2 = VBool (not (v1 == v2))

evalOp _ _ _ = throw(Error ("type error: ***")) -- Catch mismatching types and throw error
...
--------------------------------------------------------------------------------
lookupId id [] = throw (Error ("unbound variable: " ++ id)) -- Not found, throw error
lookupId id (x:xs)
  | id == fst x = snd x          -- Base: found, return value
  | otherwise   = lookupId id xs -- Recur: keep searching tail
