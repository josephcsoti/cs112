...
-- HELPERS
first :: (a, b) -> a -- Return first in pair
first (a, b) = a

second :: (a, b) -> b -- Return second in pair
second (a, b) = b

assoc :: Int -> String -> [(String, Int)] -> Int
assoc def key [] = def -- Empty Array: return def value
assoc def key (k:vs) | first k == key = second k         -- Key found: return key value
                     | otherwise      = assoc def key vs -- Recur: Find key on smaller array
...
removeDuplicates l = reverse (helper [] l)
  where
    ...
    helper seen (x:xs) = helper seen' rest'
      where
        seen' | x `elem` seen = seen        -- Seen: keep same array
              | otherwise     = [x] ++ seen -- New: add to seen array

        rest' | x `elem` seen = xs   -- Seen: Run on body (remove head)
              | otherwise     = x:xs -- New keep same array
...
wwhile :: (a -> (Bool, a)) -> a -> a
wwhile f n | first (f n) = wwhile f (second (f n)) -- Recurse: Run same function on new value given by the function
           | otherwise   = second (f n)            -- Base: Return value
...
fixpointL f x | x == f x  = [x]                      -- Base: input == output, return single element
              | otherwise = [x] ++ fixpointL f (f x) -- Recurse: Concat current and the recur result of f x
...
fixpointW f x = wwhile wwf x
 where
   wwf x = (x /= f x, -- First: Boolean, does input == output?
            f x)      -- Second: Value of applying "f" to "x"
