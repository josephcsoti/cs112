...
data Expr
  ...
  | Weird1  Expr Expr Expr -- Added expr data type
  | Weird2  Expr Expr Expr -- Added expr data type
  deriving (Show)
...
exprToString (Weird1 e1 e2 e3)    = "sin(" ++ exprToString e1 ++ "^1 +" ++ exprToString e2 ++ "^2 +" ++ exprToString e3 ++ "^3" ++ ")"
exprToString (Weird2 e1 e2 e3)    = "cos(" ++ exprToString e1 ++ "*" ++ exprToString e2 ++ "-(" ++ exprToString e1 ++ "-" ++ exprToString e3 ++ ")+(" ++ exprToString e2 ++ "-" ++ exprToString e3 ++ ")"
-- Added additional "weird" functions
...
eval x y (Weird1 e1 e2 e3)        = sin ((eval x y e1) + (eval x y e2)*(eval x y e2) + (eval x y e3)*(eval x y e3)*(eval x y e3))
eval x y (Weird2 e1 e2 e3)        = cos ( ((eval x y e1)*(eval x y e2)) - ((eval x y e1)-(eval x y e3)) + ((eval x y e2)-(eval x y e3)) )
-- Added evaluations of weird functions
...
build 0
  ...
  where
    r         = rand 10
build d       = numToExpr (rand 7) d
  where 
  numToExpr :: Int -> Int -> Expr -- Helper Func
  numToExpr n d -- Choose rand func based off of num and pass depth level
    | n==0      = Sine (build (d-1))
    | n==1      = Cosine (build (d-1))
    | n==2      = Average (build (d-1)) (build (d-1))
    | n==3      = Times (build (d-1)) (build (d-1))
    | n==4      = Thresh (build (d-1)) (build (d-1)) (build (d-1)) (build (d-1))
    | n==5      = Weird1 (build (d-1)) (build (d-1)) (build (d-1))
    | otherwise = Weird2 (build (d-1)) (build (d-1)) (build (d-1))
    -- Added "weird" functions to build
...
-- grayscale
g1, g2, g3 :: (Int, Int)
g1 = (7, 3) -- (8, 111) *Old values that took ages to run
g2 = (7, 6) -- (9, 42)
g3 = (7, 4) -- (11, 99)
...
-- grayscale
c1, c2, c3 :: (Int, Int)
c1 = (7, 2)  -- (8, 81) *Old values that took ages to run
c2 = (7, 5) -- (9, 70)
c3 = (7, 0) -- (11, 4)
